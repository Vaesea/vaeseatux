...
Stony::Stony(const ReaderMapping& reader) :
  WalkingBadguy(reader, "images/creatures/stony/stony.sprite", "left", "right"),
  m_radius()
{
  walk_speed = 80;
...
Stony::Stony(const Vector& pos, const Vector& start_pos, float vel_x, std::unique_ptr<Sprite> sprite,
                     Direction dir, float radius, const std::string& script) :
  WalkingBadguy(pos, dir, "images/creatures/stony/stony.sprite", "left", "right"),
  m_radius(radius)
{
...
void
Stony::active_update(float dt_sec)
{
...
bool
Stony::collision_squished(GameObject& object)
{
  set_action(m_dir == Direction::LEFT ? "squished-left" : "squished-right", /* loops = */ -1, ANCHOR_BOTTOM);
...
bool
Stony::is_flammable() const
{
  return false;
}

void
Stony::on_flip(float height)
{
